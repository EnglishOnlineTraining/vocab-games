...
// The exam-prep hubs — MSA and Abitur. Both are static, hand-authored pages
// (not generated by build-hub.js) whose exercises are still tracked in
// data/exercises.json under year "msa"/"abitur", so hasPart can reuse that
// data instead of re-scraping each hub's card markup.
const EXAM_PREP_HUBS = [
  { file: 'msa-activities.html', year: 'msa', name: 'MSA English Exam Preparation' },
  { file: 'abitur-activities.html', year: 'abitur', name: 'Abitur English Exam Preparation' },
];

function courseLd(hub, html) {
  const desc = decodeEntities(firstMatch(html, /<meta\s+name=["']description["']\s+content=(["'])([\s\S]*?)\1/i, 2));
  const canonical = firstMatch(html, /<link\s+rel=["']canonical["']\s+href=(["'])([\s\S]*?)\1/i, 2) || (SITE + '/' + hub.file);
  const parts = exercises.filter((e) => String(e.year) === hub.year);
  const obj = {
    '@context': 'https://schema.org',
    '@type': 'Course',
    name: hub.name,
    ...
    isAccessibleForFree: true,
    inLanguage: 'en',
    // Real delivery mode (self-paced, in-browser) — no invented schedule, price,
    // or duration, since none of those are stated on the page.
    hasCourseInstance: [{ '@type': 'CourseInstance', courseMode: 'online' }],
    hasPart: parts.map((e) => ({ '@type': 'LearningResource', name: e.title, url: SITE + '/' + e.file })),
  };
  return ldScript(obj);
}

function processExamPrepHub(hub) {
  const abs = path.join(ROOT, hub.file);
  if (!fs.existsSync(abs)) return { file: hub.file, skipped: 'missing on disk' };
  const original = fs.readFileSync(abs, 'utf8');
  const stripped = stripBlock(original, 'SCHEMA');
  const html = insertSchema(original, [courseLd(hub, stripped)]);
  if (!CHECK && html !== original) fs.writeFileSync(abs, html);
  return { file: hub.file, changed: html !== original };
}
...
const results = [
  ...
  processIndexPage(),
  ...EXAM_PREP_HUBS.map(processExamPrepHub),
];
