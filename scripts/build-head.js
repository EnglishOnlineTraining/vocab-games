 ...
 *   - a COUNT:START/COUNT:END line under a hub page's subtitle stating how
 *     many exercises that hub lists, counted from data/exercises.json
 *   - an EXPLAIN:START/EXPLAIN:END block before </body> carrying THIS page's
 *     answer explanations, so exercise.js no longer downloads all of them.
 *
...
const THEME = '#1a3a5c';
const GTM_ID = 'GTM-5HXNNPCS';

/**
 * Consent-gated Google Tag Manager.
 *
 * GTM used to load unconditionally on all 272 pages. Under GDPR Art. 6 and
 * TTDSG § 25 that is not allowed before the visitor agrees, and this is a
 * German site used by schoolchildren — so nothing reaches Google until
 * consent.js has recorded a "granted" choice.
 *
 * Two parts, both inline so they run before anything else paints:
 *   1. Consent Mode v2 defaults, every non-essential purpose denied.
 *   2. A loader that reads the stored choice and injects GTM only if it is
 *      "granted". A first-time visitor gets no GTM request at all; consent.js
 *      injects it on the spot if they then press Accept, so no reload is needed.
 *
 * eolGtmLoaded is the handshake between the two files — set here when this
 * block injects GTM, checked in consent.js so it never injects twice.
 *
 * The GTM <noscript> iframe that used to sit after <body> is deliberately gone:
 * it fired GTM for JS-off visitors, who can never answer a banner.
 */
const GTM_HEAD = `<!-- Consent Mode v2 defaults + consent-gated Google Tag Manager -->
<script>
window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}
gtag('consent','default',{ad_storage:'denied',ad_user_data:'denied',
ad_personalization:'denied',analytics_storage:'denied',
functionality_storage:'granted',security_storage:'granted',wait_for_update:500});
(function(){var c=null;try{c=localStorage.getItem('eol-consent');}catch(e){}
if(c!=='granted')return;
...
function headBlock(file, html, withSkipStyle, withOverviewStyle, withTipStyle, withRelatedStyle) {
  ...
  const lines = [
    ...
    GTM_HEAD,
    // Draws the consent banner and, on Accept, injects GTM without a reload.
    // Deferred: it only touches the DOM, and waits for window load anyway so it
    // can find the footer exercise.js injects.
    `<script src="${rel}consent.js" defer></script>`,
...
/**
 * This page's own answer explanations, inlined as the EXPLAIN global.
 *
 * Why inline rather than fetch: exercise.js used to pull the whole of
 * data/explanations.json on every page — 540 KB to use a median 3 KB entry,
 * ~99.4% waste on every load, on phones and school wifi. eolExplainForPage()
 * already prefers an inline EXPLAIN over the fetched file (it is how the
 * generated *-review.html pages work), so inlining needs no new runtime path.
 *
 * Pages with no entry get `{}` rather than nothing: that is what tells
 * exercise.js there is nothing to fetch, so the 540 KB request disappears
 * site-wide instead of only on pages that happen to have explanations. An empty
 * object is truthy, so eolExplainForPage returns it and renders no rows —
 * exactly what the fetch path did for an unknown unit.
 *
 * `<` is escaped because a `</script` sequence anywhere in the data would end
 * the element early — the browser's HTML parser does not care that it is inside
 * a JS string literal.
 *
 * Returns null for a page that already declares its own EXPLAIN (the review
 * pages, which carry generated inline data of their own); a second `var EXPLAIN`
 * would shadow theirs and blank the page's explanations.
 */
function explainBlock(html) {
  const unit = unitOf(html);
  const data = (unit && explanations[unit]) || {};
  const json = JSON.stringify(data).replace(/</g, '\\u003c');
  return `<!-- EXPLAIN:START — generated by scripts/build-head.js from data/explanations.json, do not edit by hand -->
<script>var EXPLAIN = ${json};</script>
<!-- EXPLAIN:END -->
`;
}

/* A page that declares EXPLAIN outside our own block owns its data already. */
function hasOwnExplain(html) {
  return /var\s+EXPLAIN\s*=/.test(stripBlock(html, 'EXPLAIN'));
}
...
/**
 * The GTM block is now empty apart from its markers.
 *
 * It used to hold the <noscript> iframe, which loaded GTM for visitors with
 * JavaScript off — exactly the visitors who cannot answer a consent banner.
 * The markers stay so stripBlock() keeps removing the old iframe from every
 * page that still carries it; delete them and 272 pages keep it forever.
 */
...
/**
 * The "N exercises" line under a hub page's subtitle.
 *
 * Counted from data/exercises.json via hubItems(), which is the same source the
 * JSON-LD ItemList uses — so the number a reader sees and the number a crawler
 * reads can never disagree. This exists because they did: 10c said 7 when it had
 * 14, 9g said 17 when it had 16, and uni said 11 when it had 15, each wrong in
 * the page text and in the meta description feeding the search snippet. Hand-typed
 * counts go stale the moment an exercise is added; a generated one cannot.
 *
 * activities.html is excluded — build-hub.js already writes its live #hub-count,
 * which changes as the filters narrow and is not a static total.
 */
function countBlock(file, lang) {
  if (!/-activities\.html$/.test(file)) return '';
  const n = hubItems(file).length;
  if (!n) return '';
  // msa- and grammar-activities are lang="de" and target German search traffic
  // (see the language rule in CLAUDE.md — site chrome follows the page's own
  // language), so they get "Übungen", not "exercises".
  const word = /^de/i.test(lang || '') ? (n === 1 ? 'Übung' : 'Übungen') : (n === 1 ? 'exercise' : 'exercises');
  return `<!-- COUNT:START — generated by scripts/build-head.js, do not edit by hand -->
  <p class="page-count" style="font-size:.85rem;font-weight:600;color:var(--muted);margin:-1.15rem 0 1.5rem">${n} ${word}</p>
<!-- COUNT:END -->
`;
}

/**
 * Insert the count directly after the hub's `<p class="page-subtitle">…</p>`.
 * Returns null when the page has no subtitle, which is the signal to skip it.
 */
function insertAfterSubtitle(html, block) {
  let m = html.match(/<p class="page-subtitle">[\s\S]*?<\/p>/i);
  // grammar-activities.html predates the shared hub layout and has no
  // .page-subtitle — its intro sits in a .gr-hub-hero instead.
  if (!m) m = html.match(/<div class="gr-hub-hero">[\s\S]*?<p>[\s\S]*?<\/p>/i);
  if (!m) return null;
  return insertAt(html, m.index + m[0].length, block);
}
...
function processFile(file) {
  ...
  let html = ['HEAD', 'GTM', 'NOSCRIPT', 'SKIP', 'OVERVIEW', 'RELATED', 'TIP', 'FAQ', 'EXPLAIN', 'COUNT'].reduce(stripBlock, original);
  ...
  const count = countBlock(file, meta.lang);
  let staleDesc = '';
  if (count) {
    const withCount = insertAfterSubtitle(html, count);
    if (withCount) html = withCount;
    // The tagline and meta description are hand-written prose. If either states a
  ...
  // Only the framework pages read EXPLAIN; the standalone pages have no
  // results screen to render it on.
  let hasExplain = false;
  if (framework && !hasOwnExplain(html) && /<\/body>/i.test(html)) {
    html = html.replace(/<\/body>/i, explainBlock(html) + '</body>');
    hasExplain = true;
  }

  if (!CHECK && html !== original) fs.writeFileSync(abs, html);
  return { file, changed: html !== original, noscript: needsNoscript, skip: wantsSkip, noLandmark, explain: hasExplain, staleDesc };
}
...
const withExplain = results.filter((r) => r.explain);
...
console.log(`build-head: ${files.length} pages scanned, ${changed.length} ${CHECK ? 'would change' : 'updated'}, ` +
  ...
  `${withExplain.length} carry inline explanations.`);
results.filter((r) => r.staleDesc).forEach((r) => console.log(`  ⚠️  ${r.file}: ${r.staleDesc}`));
