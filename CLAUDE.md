...
**To add explanations for a page:** append `"<unit>": { … }` to `data/explanations.json` (get the exact `<select>` ids/answers with the extractor, and use the page's real `var UNIT` as the key — it is not always the filename, e.g. `tudor-past-perfect`), then run **`node scripts/validate-explanations.js`** (checks every `prefix+gap` id exists and each `correct`/`accept` is a real option). Done pages: `10g-scottish-history`, `9g-famous-hollywood`, `7g-tudor-past-perfect`, `10c-government-systems` (69 gaps). Abitur packs are a separate architecture (see TEMPLATE-NOTES) and need their own path.
