...
**`sitemap.xml` has no `<lastmod>`, and adding it needs a pipeline change first (investigated 2026-09-05).** Worth having: without it all 257 URLs look equally stale to Google, which is the signal it uses to decide what to re-crawl on a domain where 145 pages sit at "Discovered – currently not indexed". Two approaches were tried and both failed; don't repeat them.

- **Dating from `git log` breaks on clone depth.** The obvious implementation (last commit that touched each file) is wrong in a shallow checkout: `git log` sees one commit, every path resolves to it, and all 257 URLs get the same date — a sitemap that is entirely wrong while looking well-formed. `check-generated.yml` checks out at the default depth 1, so `--check` failed reporting only `M sitemap.xml` with nothing in the diff explaining why. **This container's own clone is shallow too** (186 commits, truncated at 2026-08-13), so local builds are not a safe test of it. `fetch-depth: 0` on the gate would fix CI but leaves the generator quietly wrong anywhere else. File mtime is worse still — a CI checkout stamps every file with the checkout time.
- **Hashing page content into a committed date file breaks on generator order.** The robust design — `data/lastmod.json` mapping path → `{date, hash}`, date bumped only when the hash changes — needs the hash taken *after* the last generator has written the page. `build-head.js` runs **after** `build-topic-pages.js` and rewrites heads, so a hash taken during topic-pages goes stale and the dates churn on the next build.

So the fix is a **late pipeline node** (or folding the pass into `build-head.js`, which already runs last and already scans every page): recompute hashes once everything is written, then rewrite `sitemap.xml`'s `lastmod` values. That is a real change to `pipeline.js`, not a tweak to the sitemap line, which is why it was not done as part of the reciprocal-links work.
