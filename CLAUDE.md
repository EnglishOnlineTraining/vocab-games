...
**First-answer scoring (added 2026-07-17).** The recorded score counts each gap's **first checked answer** only, tracked per-gap in `state.firstTry[scoreKey]`. A gap is locked in the first time it is checked with a non-blank value; blank gaps stay unrecorded until the student actually answers and checks them. Re-checking still recolours the gaps and shows current feedback, so students can learn from mistakes — but changing an answer after a check can no longer improve the score sent to the teacher. When the visible result differs from the recorded score, the feedback line and the score card both say so ("Your recorded score counts your first answers: X / Y"). This lives in `checkDropdowns()` in `exercise.js` (the single shared copy — see "Shared framework" below); `9g-india-phrasal-verbs.html` additionally has a bespoke first-try multiple-choice checker in its `checkExA()`. Pages that pass no `scoreKey` never record or send a score, so they are unaffected. `test-scoring.js` (run with `node test-scoring.js`) is the self-check for this logic.
...
**IT series (converted 2026-07-17).** The 10 `it-*` exercise pages were rebuilt from their bespoke ES6 framework onto the standard shared framework and house style. Changes: students now enter **name + course/group on the welcome step** (previously name + email at the end); submissions go to the **Business English/University Apps Script sheet** (the universal handler auto-creates one tab per `it-*` unit) instead of FormSubmit.co, so the student-email/reply-to field is gone and the email fallback button covers direct contact; they gained the step-nav bar, copy/paste blocking, first-answer scoring, and the Score + Note card. Their content-specific CSS classes (`level-badge`, `section-badge`, `section-instructions`, `gap-sentence`, `q-num`) were added to `style.css` under "IT-series content".
