...
**German text carries `lang="de"` — mark it when you add it (2026-08-27).** `activities.html` and
`index.html` are `<html lang="en">` but most of the hub chrome is German, so a screen reader was
reading "Finde deine Übung" and "Gemischte Wiederholung" with English phonemes (WCAG 2.2 SC 3.1.2).
`build-hub.js` now marks it: `de(s)` wraps German in a `<span lang="de">` for slots that take raw
HTML, and elements whose German lives in an *attribute* (`placeholder`, `aria-label`) carry
`lang="de"` inline, since an attribute cannot be wrapped. `YEAR_LANG` says which `YEAR_LABEL` values
are German — 'IT English' / 'Business English' are not, and proper names (Gymnasium, Oberschule,
Abitur, MSA, Quiz) are exempt under 3.1.2 and stay unmarked. The hub `<section>` swapped its German
`aria-label` for `aria-labelledby="hub-h"` so the accessible name inherits the heading's `lang`.
**Deliberately not marked by section:** 179 of the 201 exercise titles are English, so a
`lang="de"` on the whole `.hub` would be worse than the bug. The remaining gap is the 22
part-German *titles* in `data/exercises.json` (`… — Wiederholung`, `… – Interaktive Übung`) — no
rule separates their German half from their English half reliably, so they need a per-exercise
title language in the data if they are ever to be marked. The same applies outside the generator:
the two German banners and the footer in `activities.html` are hand-maintained and carry `lang="de"`
directly, and `build-head.js`'s skip link marks its German half.
